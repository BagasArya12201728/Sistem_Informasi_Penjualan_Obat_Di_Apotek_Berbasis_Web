...
//END data obat ---------------------------------------------------------------------------------------------------------------------------------------
        ...
        $kode = $_GET['kode'];
        $sql1 = $connect->query("DELETE FROM detail WHERE kode_obat = '$kode'");
    ...
    echo "<meta http-equiv='refresh' content='0; url=../pages/view/struk?kode=$kode_trans'>";
        ...
        $sql = $connect->query("INSERT INTO db_user (username, nama, password, akses, alamat)
        ...
        $sql = $connect->query("DELETE FROM db_user WHERE username = '$username'");
        if ($sql) {
            header("location:../home?p=user&&status=sukses");
        }else{
            header("location:../home?p=user&&status=gagal");
        ...
        $sql = $connect->query("UPDATE db_user SET nama='$nama', akses='$level', alamat='$alamat', password='$pass' WHERE username = '$username' ");
        if ($sql) {
            header("location:../home?p=user&&status=sukses");
        }else{
            header("location:../home?p=user&&status=gagal");
